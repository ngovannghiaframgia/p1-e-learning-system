$(document).on('turbolinks:load', function() {

  $("#formattedNumberField").on("input", function(evt) {
    var self = $(this);
    self.val(self.val().replace(/[^0-9\.]/g, ''));
    if ((evt.which != 46 || self.val().indexOf('.') != -1) && (evt.which < 48 || evt.which > 57))
    {
      evt.preventDefault();
    }
    if ($(this).val() != "") {
      var n = parseInt($(this).val().replace(/\D/g,''),10);
      $(this).val(n.toLocaleString().replace(/\,/g, '.'));
    }
  });

  if($('#formattedNumberField').length && $('#formattedNumberField').val() != "" ){
    var n = parseInt($('#formattedNumberField').val().replace(/\D/g,''),10);
    $('#formattedNumberField').val(n.toLocaleString().replace(/\,/g, '.'));
  }

  if($('.course-price').length){
    for (let i = 0; i < $('.course-price').length; i++) {
      var n = parseInt($('.course-price').eq(i).text().replace(/\D/g,''),10);
      $('.course-price').eq(i).text(n.toLocaleString().replace(/\,/g, '.'));
    }
  }
})
